...
from db.models import user_model,competition_model,match_model
from core import schemas

# --- User CRUD ---

def create_user(db: Session, user: schemas.user_schema.UserCreate):
    db_user = user_model.User(username=user.username, email=user.email, is_player=user.is_player)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
def get_players(db: Session, skip: int = 0, limit: int = 100):
    return db.query(user_model.User).filter(user_model.User.is_player == True).offset(skip).limit(limit).all()

def get_player(db: Session, player_id: int):
    return db.query(user_model.User).filter(user_model.User.id == player_id, user_model.User.is_player == True).first()

# --- Competition CRUD ---

def create_competition(db: Session, comp: schemas.competition_schema.CompetitionCreate):
    db_comp = competition_model.Competition(name=comp.name)
    db.add(db_comp)
    db.commit()
    db.refresh(db_comp)
    return db_comp
...
def get_competitions(db: Session, skip: int = 0, limit: int = 100):
    return db.query(competition_model.Competition).offset(skip).limit(limit).all()

def get_competition(db: Session, competition_id: int):
    return db.query(competition_model.Competition).filter(competition_model.Competition.id == competition_id).first()
...
def get_matches(db: Session, skip: int = 0, limit: int = 100, competition_id: int = None):
    query = db.query(mach_model.match)
    if competition_id:
        query = query.filter(mach_model.match.competition_id == competition_id)
    return query.offset(skip).limit(limit).all()

def update_match(db: Session, match_id: int, match_data: schemas.match_schema.MatchUpdate):
    db_match = db.query(mach_model.match).filter(mach_model.match.id == match_id).first()
    if not db_match:
        return None
